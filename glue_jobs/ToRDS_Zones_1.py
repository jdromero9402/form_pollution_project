"""
AWS Glue Job - PARTE 2-1: CARGAR ZONES A RDS
Lee zones.parquet desde S3 y carga a RDS con IAM Auth
...
def main():
    ...
    try:
        ...
        print("\n📂 PASO 1: Leyendo zones.parquet desde S3...")
        ...
        try:
            ...
            required_cols = ['zone_id', 'city_code', 'city_name', 'zone_code', 
                           'zone_name', 'population', 'centroid_lat', 'centroid_lon']
        ...
        verify_table_exists(conn, SCHEMA, 'zones')
        ...
        try:
            ...
            df_pandas['population'] = df_pandas['population'].astype(int)
            df_pandas['centroid_lat'] = df_pandas['centroid_lat'].astype(float)
            df_pandas['centroid_lon'] = df_pandas['centroid_lon'].astype(float)
            
            #records = df_pandas.to_records(index=False)
        ...
        INSERT INTO {SCHEMA}.zones 
            (zone_id, city_code, city_name, zone_code, zone_name, 
        ...
        with conn.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) FROM {SCHEMA}.zones")
