"""
AWS Glue Job - PARTE 2-2: CARGAR STATIONS A RDS
Lee stations.parquet desde S3 y carga a RDS con IAM Auth
...
print("=" * 70)
print("PARTE 2-1: CARGANDO ZONES A RDS")
print("=" * 70)
...
def get_rds_connection(retry_count=0):
    ...
    try:
        ...
        # Validar parámetros
        if not args['RDS_HOST'] or not args['RDS_USER']:
            raise ValueError("RDS_HOST y RDS_USER son obligatorios")
...
def main():
    ...
    try:
        ...
        print("\n📂 PASO 1: Leyendo stations.parquet...")
        ...
        try:
            ...
            required_cols = ['station_id', 'station_code', 'station_name', 'zone_id',
                           'lat', 'lon', 'station_type', 'authority', 'years_min', 'years_max']
        ...
        verify_table_exists(conn, SCHEMA, 'stations')
        ...
        try:
            ...
            df_pandas['station_id'] = df_pandas['station_id'].astype(int)
            df_pandas['zone_id'] = df_pandas['zone_id'].astype(int)
            df_pandas['lat'] = df_pandas['lat'].astype(float)
            df_pandas['lon'] = df_pandas['lon'].astype(float)
            df_pandas['years_min'] = df_pandas['years_min'].astype(int)
            df_pandas['years_max'] = df_pandas['years_max'].astype(int)
        ...
        INSERT INTO {SCHEMA}.stations 
            (station_id, station_code, station_name, zone_id, lat, lon, 
             station_type, authority, years_min, years_max)
        ...
        with conn.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) FROM {SCHEMA}.stations")
