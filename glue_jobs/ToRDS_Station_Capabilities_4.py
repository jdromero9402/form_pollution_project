"""
AWS Glue Job - PARTE 2-4: CARGAR STATION_CAPABILITIES A RDS
Lee station_capabilities.parquet desde S3 y carga a RDS con IAM Auth
...
print("=" * 70)
print("PARTE 2-4: CARGANDO STATION_CAPABILITIES A RDS")
print("=" * 70)
...
def main():
    ...
    try:
        ...
        print("\n📂 PASO 1: Leyendo station_capabilities.parquet...")
        ...
        try:
            ...
            required_cols = ['station_id', 'pollutant_id', 'unit']
        ...
        verify_table_exists(conn, SCHEMA, 'station_capabilities')
        ...
        try:
            ...
            df_pandas = df_pandas.drop_duplicates(subset=['station_id', 'pollutant_id'])
            duplicates_removed = original_count - len(df_pandas)
        ...
        sql = f"""
        INSERT INTO {SCHEMA}.station_capabilities 
            (station_id, pollutant_id, unit)
        VALUES (%s, %s, %s)
        ON CONFLICT (station_id, pollutant_id) DO UPDATE SET
            unit = EXCLUDED.unit
        """
        ...
        with conn.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) FROM {SCHEMA}.station_capabilities")
